<h1 align="center">
Hi 👋, I'm <span style="color:#00C4FF;">Urwa Tul Wusqa</span>
</h1>

<h3 align="center">
🚀 MERN Stack Developer | React | Node.js | Express | MongoDB
</h3>

<p align="center">
Building clean, responsive and scalable web applications.
</p>

<p align="center ">

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&size=24&duration=3000&pause=1000&center=true&vCenter=true&width=700&lines=Full+Stack+MERN+Developer;React+Developer;Node.js+Developer;Always+Learning+New+Technologies;Open+To+Remote+Opportunities"/>

</p>


---

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0F172A,100:38BDF8&height=2&section=header"/>



## 👩‍💻 About Me

Hey there! 👋 I'm **Urwa Tul Wusqa**, a **Full-Stack MERN Developer** from Pakistan 🇵🇰.

💡 I enjoy transforming ideas into responsive, user-friendly, and scalable web applications.

🚀 Every project helps me improve my skills in frontend development, backend architecture, and writing clean, maintainable code.

🌱 Currently learning advanced MERN concepts while building projects that solve real-world problems.

✨ My goal is simple: keep learning, keep building, and create products that people genuinely enjoy using.

---

## 🚀 What I'm Up To

🌐 Building and improving my personal portfolio

🛍️ Developing **MedWear**, a MERN e-commerce platform

📚 Exploring backend architecture & API development

💼 Looking for remote Full-Stack MERN opportunities

☕ Turning coffee into clean code

---

## ⚡ Fun Fact

```javascript
const urwa = {
    code: ["JavaScript", "React", "Node.js"],
    challenge: "Building better projects every day 🚀",
    motto: "Learn • Build • Improve • Repeat 🔥"
};


```
---




## 🛠️ Tech Stack

> 💙 **Favorite Stack:** React + Node.js + MongoDB + Tailwind CSS

### 🎨 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,vite,html,css,js,tailwind,redux" />
</p>

### ⚙️ Backend
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express" />
</p>

### 🗄️ Database
<p>
  <img src="https://skillicons.dev/icons?i=mongodb" />
</p>

### ☁️ Tools & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,npm,vercel" />
</p>

### 🌱 Currently Exploring
<p>
  <img src="https://skillicons.dev/icons?i=docker,firebase" />
</p>

---

## ⚙️ Tech I Enjoy Working With

<p align="center">

<img src="https://skillicons.dev/icons?i=react,nodejs,express,mongodb,js,tailwind,redux,git,github,vscode,postman,firebase&perline=6"/>

</p>

> "Every project is an opportunity to learn something new and build something better."

---

## 📊 GitHub Analytics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=urtuqa6013-web&show_icons=true&theme=tokyonight&hide_border=true&border_radius=12&rank_icon=github"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=urtuqa6013-web&layout=compact&theme=tokyonight&hide_border=true&border_radius=12"/>

</div>

<div align="center">

<img src="https://streak-stats.demolab.com?user=urtuqa6013-web&theme=tokyonight&hide_border=true&border_radius=12"/>

</div>




---


# 📈 Contribution Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=urtuqa6013-web&theme=tokyo-night"/>

</p>


---

## 🚀 Featured Projects

<div align="center">

<a href="https://github.com/urtuqa6013-web/medwear">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=urtuqa6013-web&repo=medwear&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/urtuqa6013-web/connecthub">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=urtuqa6013-web&repo=connecthub&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/urtuqa6013-web/portfolio">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=urtuqa6013-web&repo=portfolio&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/urtuqa6013-web/task-manager">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=urtuqa6013-web&repo=task-manager&theme=tokyonight&hide_border=true" />
</a>

</div>

## 🌟 What You'll Find Here

💊 **MedWear**
> Full-stack MERN e-commerce platform with authentication, shopping cart, orders, reviews, and an admin dashboard.

💼 **Portfolio**
> A modern portfolio showcasing my projects, skills, and developer journey.

💬 **ConnectHub**
> Real-time chat application with authentication and Socket.IO.

📋 **Task Manager**
> Task management application featuring JWT authentication, role-based access, search, filtering, and pagination.


## 💙 Current Favorite Project

🛍️ **MedWear**

Building MedWear has helped me gain hands-on experience with:

- 🔐 JWT Authentication
- ☁️ Cloudinary Image Uploads
- 💳 Order Management
- ⭐ Product Reviews
- 📊 Admin Dashboard
- ⚡ REST APIs
- 🗄️ MongoDB & Mongoose

---

## 🌐 Let's Connect

<div align="center">

<a href="https://www.linkedin.com/in/urwa-tul-wusqa-39029341b">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://portfolio-urwa-tul-wusqa.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<a href="mailto:urtuqa6013@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/urtuqa6013-web">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

## 💡 My Development Philosophy

```javascript
while (alive) {
    learn();
    build();
    improve();
    repeat();
}
```

> "I believe the best way to learn is by building projects, solving real problems, and continuously improving with every commit."

---

## ✨ Daily Inspiration

![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

---

## 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/urtuqa6013-web/urtuqa6013-web/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />

</div>

---


## 💼 My Journey

```text
2025  🌱 Started learning Web Development
   │
   ├── Learned HTML, CSS & JavaScript
   │
2026  ⚛️ Built projects with React & Tailwind CSS
   │
   ├── Learned Node.js & Express
   │
   ├── Worked with MongoDB & REST APIs
   │
   ├── Developed MERN Stack applications
   │
   └── 🚀 Open to Full-Stack MERN Opportunities

```

## 📚 Learning & Growth

- 🎓 MERN Stack Development
- 📖 REST API Development
- 🔐 JWT Authentication
- ☁️ Cloudinary Integration
- 📦 Redux Toolkit
- 🎨 Responsive UI Design
- 🌱 Currently exploring Docker & deployment

## 🎯 What I Value

✨ Writing clean and maintainable code

🤝 Teamwork and communication

📱 Responsive and accessible UI

🚀 Continuous learning

💡 Building products that solve real problems

## ☕ A Little More About Me

- 🌙 I enjoy late-night coding sessions.
- 💡 I love turning ideas into working products.
- 📚 I believe every project teaches something new.
- 🎯 My goal is to become a skilled Full-Stack Engineer who builds impactful applications.

## ⚡ Skills Snapshot

| Category | Technologies |
|----------|--------------|
| 🎨 Frontend | React, Vite, Tailwind CSS, Redux |
| ⚙️ Backend | Node.js, Express.js |
| 🗄️ Database | MongoDB, Mongoose |
| 🛠️ Tools | Git, GitHub, VS Code, Postman |
| 🚀 Deployment | Vercel, Render |


## 📈 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=urtuqa6013-web&bg_color=0d1117&color=38bdf8&line=38bdf8&point=ffffff&area=true&hide_border=true&radius=16"/>

</div>


## 🚀 Build Showcase

<table>
<tr>

<td width="50%">

### 🛍️ MedWear

Full-stack MERN e-commerce platform for medical apparel.

✨ Features

- JWT Authentication
- Admin Dashboard
- Product Reviews
- Shopping Cart
- Order Management
- Cloudinary Upload
- Responsive UI

**Tech Stack**

`React` `Node.js` `Express` `MongoDB`

</td>

<td width="50%">

### 🌐 Portfolio

Modern developer portfolio with animations.

✨ Features

- Responsive Design
- Smooth Animations
- Contact Form
- Project Showcase
- Dark Theme

**Tech Stack**

`React` `Vite` `Tailwind`

</td>

</tr>
</table>


<div align="center">

LinkedIn | Portfolio | Gmail | GitHub

</div>

